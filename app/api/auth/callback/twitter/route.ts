...
export async function GET(req: NextRequest) {
  try {
    ...
    const tokenData = await exchangeCodeForToken('twitter', code, state)
    const profile = await getPlatformUserProfile('twitter', tokenData.accessToken)
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/dashboard/social?tab=connections&success=twitter`
    )
  } catch (error: any) {
